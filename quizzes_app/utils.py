...
def _get_whisper_model():
    """
    Loads the Whisper model once and caches it for subsequent calls.
    Avoids reloading the model (~460 MB) on every transcription request.
    Uses CUDA (GPU) if available for significantly faster transcription.
    """
...
def _extract_caption_url(captions: dict) -> str:
    """
    Returns the URL of the preferred subtitle language.
    Prioritizes English, then German. Falls back to the first available language
    if neither is found.

    Args:
        captions: Dictionary of available captions keyed by language code.

    Returns:
        URL string pointing to the subtitle file.
    """
    for lang in ["en", "de"]:
        if lang in captions:
...
def _parse_caption_response(response_text: str) -> str:
    """
    Parses the raw YouTube subtitle JSON response into a clean text string.
    Extracts all text segments, strips whitespace, and removes duplicate words
    to produce a readable transcript.

    Args:
        response_text: Raw JSON string from the YouTube subtitle URL.

    Returns:
        A cleaned, deduplicated transcript string.
    """
...
def get_youtube_transcript(url: str) -> str | None:
    """
    Attempts to fetch existing YouTube subtitles for the given video URL.
    Checks for manual subtitles first, then falls back to auto-generated captions.

    Args:
        url: A valid YouTube video URL.

    Returns:
        The transcript as a string if subtitles are available, otherwise None.
    """
...
def download_youtube_audio(url: str) -> str:
    """
    Downloads the audio track of a YouTube video and converts it to MP3 via ffmpeg.
    Saves the file to a temporary directory that the caller is responsible for cleaning up.
...
def transcribe_audio(audio_path: str) -> str:
    """
    Transcribes a local audio file using Whisper AI.
    Uses the cached Whisper model to avoid reloading on every call.
...
def get_video_transcript(url: str) -> str:
    """
    Main entry point for transcript retrieval.
    Attempts to fetch existing YouTube subtitles first (fast path).
    Falls back to downloading the audio and transcribing it with Whisper AI
    ...
    """
    print("Searching for existing YouTube subtitles...")
    ...
    print("No subtitles found — falling back to Whisper AI transcription.")
